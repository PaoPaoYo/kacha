...
- 两段式框选（2026-09-15 用户迭代需求，取代初版「松开即截」）：
  ...
  2. 调整态：四角锚点拖动双向缩放；**拖动任意一条边**沿该边单向缩放（整条边线为命中区，四角优先）；选区内拖动整体平移；**选区外拖动/点击什么也不做**（重新框选须 ESC 后重来）；调整中保持最小 4pt
  3. 确认：双击选区内、回车、或点右下角操作按钮（「复制」/「保存」）；ESC 随时取消整个流程
  4. 调整态右下角液态玻璃文字按钮组：「保存」（NSSavePanel 选位置存 PNG）+「复制」（入剪贴板），位置自动避让屏幕边缘
