...
### Task 9: 保存按钮（复用选择链路扩展 save 动作）（2026-09-15 用户迭代需求）

**Files:**
- Modify: `Sources/Kacha/SelectionView.swift`
- Modify: `Sources/Kacha/OverlayController.swift`
- Modify: `Sources/Kacha/CaptureCoordinator.swift`
- Modify: `Sources/Kacha/ClipboardService.swift`

**Interfaces:**
- Consumes: Task 7/8 的 SelectionView 结构（Phase/confirm()/CursorState/applyCursor/inChildZone gate）、OverlayController.show、ClipboardService.write
- Produces:
  - `enum CaptureAction { case copy, save }`（OverlayController.swift 文件级）
  - `OverlayController.show(frames:onCapture:onCancel:)`，其中 `onCapture: @escaping (CGImage, CaptureAction) -> Void`
  - `SelectionView` 新增 `let onSave: (CGRect) -> Void`（onConfirm 保持复制语义）
  - `ClipboardService.pngData(_ image: CGImage) -> Data?`（静态，供保存复用）

**规格：**
- 复制按钮左侧新增同规格 32×32 液态玻璃按钮，图标 `square.and.arrow.down`，间距 12pt，位置公式覆盖两钮（右缘/左缘 clamp、下方不足收内侧，两钮同 y）
- 点击保存：关闭全部覆盖窗 → NSSavePanel（默认名「截图 yyyy-MM-dd HH.mm.ss.png」、allowedContentTypes = [.png]、canCreateDirectories = true）→ 写文件 → Tink 声；取消面板静默结束（不写不响）
- 光标 pointingHand 判定覆盖两钮（buttonFrame → buttonFrames）；inChildZone gate 覆盖两钮
- 保存失败（写文件错误）presentError alert
- 提交信息：`feat: 保存按钮，选择位置保存 PNG`，末尾 Co-Authored-By: Claude Code <noreply@anthropic.com>
...
// 按钮组中心（单一公式源，替代 Task 8 的 badgeCenter）：
...
// save 按钮点击 → guard phase == .adjusting, dragStart == nil, isValid → onSave(selection)
```

OverlayController：show 的 onCapture 闭包带 CaptureAction；SelectionView 初始化传 onConfirm（→handleConfirm action .copy）与 onSave（→ .save）。
...
private func saveToFile(_ image: CGImage) {
    ...
    guard let data = ClipboardService.pngData(image) else { presentError(...); return }
    do { try data.write(to: url); NSSound(named: NSSound.Name("Tink"))?.play() }
    catch { presentError(error) }
}
```
（import UniformTypeIdentifiers；presentError 复用已有方法）

- [ ] Step 1: 实现四个文件改动（SelectionView 按钮组+光标+gate；OverlayController 回调链；CaptureCoordinator saveToFile；ClipboardService 提取 pngData）
- [ ] Step 2: `swift build && swift test`（12/12 仍过）→ `make app`
- [ ] Step 3: 用户 GUI 验证（保存按钮显示/位置/点击→面板→文件落地/取消/复制仍正常/光标）
- [ ] Step 4: Commit（信息见上）
